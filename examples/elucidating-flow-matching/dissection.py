...
D.statement("c_denoiser_geometry", "The denoiser turns data geometry into attraction, absorption and posterior concentration", "The denoiser is the only data-dependent part of the vector field; its attracting and absorbing behaviour (Theorems 3.1-3.2) and the concentration of the posterior (Theorem 5.1) are the framework behind every stage of the trajectory.", warrant="interpreted", function="theory_answer", spans=D.sp("s0030"), echoes=D.sp("s0006", ("s0037", "s0038"), "s0395"))
D.statement("c_three_stages", "FM trajectories exhibit mean, cluster, and terminal stages", "The trajectory first aligns with the data mean, then responds to local clusters, and finally converges toward the fine-scale support.", warrant="interpreted", function="theory_answer", spans=D.sp(("s0031", "s0034")), echoes=D.sp("s0007", "s0039", ("s0179", "s0182")))
...
D.statement("c_terminal_consequences", "Terminal analysis yields equivariance and a memorization mechanism", "Once the terminal map is controlled, the paper derives similarity equivariance and explains why an asymptotically empirical denoiser reproduces training atoms.", warrant="interpreted", function="theory_answer", spans=D.sp("s0009"))
...
D.statement("c_first_submanifold", "First terminal convergence result covering submanifold-supported data", "The authors position Theorem 5.3 as the first terminal convergence result that accommodates data supported on submanifolds.", warrant="interpreted", function="comparison", spans=D.sp("s0041"))
D.statement("intro_equivariance", "The terminal map is equivariant under similarities", "Appropriately transformed schedules make the flow map commute with scaling, rotation, and translation. Stated in the introduction only; Proposition 5.7 is the formal statement.", warrant="interpreted", function="theory_answer", spans=D.sp("s0042"))
D.statement("intro_memorization", "Near-terminal empirical denoisers expose memorization", "For discrete empirical targets, terminal-time behavior ties learned denoiser optimality to convergence onto training examples: to generalize, the near-terminal denoiser itself must generalize.", warrant="interpreted", function="theory_answer", spans=D.sp("s0337"), echoes=D.sp("s0043"))
...
D.statement("c_angle_attracts", "A quantitative acute-angle bound forces attraction", "Controlling the projection-angle error yields decay of distance to the set.", warrant="interpreted", function="theory_answer", spans=D.sp("s0146"), story=False)
...
D.statement("c_convex_hull", "Every FM path is pulled toward the data convex hull", "Because posterior means lie in the convex hull of the support, that hull is attracting and absorbing.", warrant="interpreted", function="theory_answer", spans=D.sp(("s0165", "s0168")), story=False)
...
# ---- Preterminal stages --------------------------------------------------------
D.statement("c_wellposed", "FM ODEs are well posed on every preterminal interval", "A finite second moment suffices for a unique path from every initial point and a continuous flow transporting the prior to p_t for t<1.", warrant="interpreted", function="theory_answer", spans=D.sp("s0184"), story=False)
...
D.statement("c_mean", "At large noise, trajectories move toward the data mean", "Because the denoiser approaches the data mean at the initial endpoint, a quantitative high-noise interval contracts mean distance.", warrant="interpreted", function="theory_answer", spans=D.sp(("s0193", "s0194")), story=False)
...
D.statement("c_cluster", "At intermediate noise, coarse clusters capture trajectories", "For a separated local cluster, nearby trajectories are absorbed near its convex hull and converge toward that hull.", warrant="interpreted", function="theory_answer", spans=D.sp(("s0205", "s0207")), story=False)
...
D.statement("c_to_terminal", "Fine-scale support geometry takes over as noise vanishes", "Near zero noise, attraction shifts from coarse clusters toward nearest support points or manifold geometry.", warrant="interpreted", function="interpretation", spans=D.sp(("s0228", "s0229")), story=False)
...
D.statement("m_posterior_rate", "Posterior concentration supplies a usable denoiser rate", "Geometry-dependent posterior rates strengthen absorption, keep paths off the medial axis, and make their terminal motion integrable.", warrant="derived", function="setup", spans=D.sp(("s0242", "s0246")), echoes=D.sp("s0046"))
...
D.target("q_memorization", "What must change near terminal time for the model to generalize?", "Identify why an empirical optimum memorizes and what a learned denoiser must do differently.", subtype="research_question", spans=D.sp("s0338"))
...
D.edge("c_wellposed", "c_three_stages", "supports")
...
D.edge("r_terminal", "c_first_submanifold", "supports")
D.edge("c_to_terminal", "c_three_stages", "supports")
...
D.edge("c_terminal_answer", "c_terminal_consequences", "develops", ev=["s0009"])
D.edge("intro_equivariance", "c_terminal_consequences", "develops", ev=["s0009"])
D.edge("intro_memorization", "c_terminal_consequences", "develops", ev=["s0009"])
D.edge("c_denoiser_geometry", "c_three_stages", "develops", ev=["s0038"])
D.edge("c_denoiser_geometry", "c_terminal_answer", "develops", ev=["s0038"])
D.edge("r_denoiser_projection", "c_denoiser_geometry", "supports")
D.edge("c_terminal_answer", "q_equivariance", "motivates", ev=["s0042", "s0293"])
